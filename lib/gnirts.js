...
(new RegExp(
  '^[\\s\\S]{INDEX}' +
  (CODE_STRING).replace(/([META])/g, function(s, meta) {
  ...
  while (index < str.length) {
    ...
    arrCode.push(codeRe ?
      ...
      '(new RegExp(\'^[\\\\s\\\\S]{' + index + '}\'+(' +
        // `\W` in a byte
        codePart + ').replace(/([\\x00-\\x2f\\x3a-\\x40\\x5b-\\x5e\\x60\\x7b-\\x7f])/g,function(s,I){return \'\\\\x\'+(\'00\'+I.charCodeAt().toString(16)).substr(-2)}))).test(' +
        target + ')' :
      '(' + target + ').indexOf(' + codePart + ',' + index + ')===' + index
    );
    codeRe = !codeRe;
    index += partLen;
  }
...
exports.mangle = function(source) {
  ...
  return source.replace(reDir, function(s, content) {
    ...
    if ((matches = reLiteral.exec(content))) {        // ======== Literal
      ...
      return (matches[1] || '') +
        getCode4match(matches[2], getStringFromCode(matches[3])) + (matches[4] || '');
    } else {
